...
- When a SwiftUI control positions interactive affordances with `offset`, do not assume the parent layout or gesture bounds expand with the rendered position. Explicitly frame the gesture host to the full intended hit region or trailing controls can become visible but non-draggable.
- When a SwiftUI timeline already relies on one parent drag gesture for stable handle routing, do not bolt a second child drag gesture onto an overlapping indicator. Resolve the drag target centrally at drag-begin, or the parent gesture can starve the child interaction.
